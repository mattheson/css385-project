...
public class GameController : MonoBehaviour
{
    ...
    // called by character when player punch the fitness equipments
    // increase walking and running speed
    public void increaseSpeed()
    {
        // maximum speed the player could reach
        const float maxWalkingSpeed = 10f;
        const float maxRunningSpeed = 15f;

        if (Game.walkingSpeed < maxWalkingSpeed)
        {
            Game.walkingSpeed += 1;
            if (Game.walkingSpeed > maxWalkingSpeed)
            {
                Game.walkingSpeed = maxWalkingSpeed;
            }
        }

        if (Game.runningSpeed < maxRunningSpeed)
        {
            Game.runningSpeed += 1;
            if (Game.runningSpeed > maxRunningSpeed)
            {
                Game.runningSpeed = maxRunningSpeed;
            }
        }
    }
